...
def main(config):
    ...
    if not math.isfinite(offload_min_mb) or offload_min_mb <= 0:
        raise ValueError("activation_cpu_offload_min_mb must be positive and finite")
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            pixel_loss_context = (
                SelectiveActivationOffload(int(offload_min_mb * 2**20), offload_pin_memory)
                if activation_cpu_offload and config.run.pixel_space_supervise else nullcontext()
            )
